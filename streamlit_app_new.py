...
# ==================== v3.3 原始默认参数 ====================
DEFAULT_V33 = {
    'preset_key': '通用荧光颗粒',  # 对应老版本通用场景
    'algo_multiselect': ['LoG 斑点检测', '局部最大值'],  # 老版本默认开的两种
    'sens_slider': 50,       # 平衡模式
    'size_slider': 8,        # 默认尺寸级别
    'unif_slider': 20,       # uniform_size=False → 大小不一
    'dist_slider': 6,        # 平衡模式默认间距
    'color_select': '红色',   # 老版本默认红色
    'results': [],
    'current_idx': 0,
    'has_run': False,
}
...
def reset_to_v33_defaults():
    """一键恢复 v3.3 原始默认参数"""
    for k, v in DEFAULT_V33.items():
        st.session_state[k] = v
    st.toast("已恢复 v3.3 默认参数：红色通道 / 平衡模式 / LoG+局部最大值", icon="🔄")
...
st.sidebar.button(
    "🔄 恢复 v3.3 默认参数",
    on_click=reset_to_v33_defaults,
    type="secondary",
    use_container_width=True,
    help="恢复最早版本默认配置：红色通道 / 平衡模式 / LoG+局部最大值 / 大小不一"
)
